...
function SelectDate(obj, strFormat, x, y) {
    ...
    var date = new Date();
    var by = date.getFullYear() - 70;  //最小值 → 50 年前   
...
Calendar.prototype.ReturnDate = function (dt) {
    ...
    try {
        if ($(this.dateControl).next(".publicError").length > 0) {//jack
            $(this.dateControl).next(".publicError").remove();
        }
    } catch (err) {

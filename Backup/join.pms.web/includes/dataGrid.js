...
function SetCheckBoxClear(objID){  
    //var cbx = document.getElementsByName("ItemChk");
    //var itemsAry = document.form1.ItemChk;
    var itemsAry = window.document.getElementsByName("ItemChk");
    for(var i=0; i<itemsAry.length; i++)     
    {         
...
function ChangeUrl(objUrl,objName,objAction)
{
    if(objUrl.length > 0)
    {
        ...
        else
        {
            ...
            else if(objAction=="execel")
            {
                window.location.href = objUrl + "&sourceUrl="+sourceUrl+"&oNa="+objName;
            }
            else if(objAction=="dataimport")
            {
                window.location.href = objUrl + "&sourceUrl="+sourceUrl+"&oNa="+objName;
            }
            else
    ...
    if(objUrl.length > 0)
    {
        ...
        else
        {
            ...
            if(CheckChangeParams(objFuncNo,objParams)>0)
            {
                ...
                if(objUrl.indexOf("action=chexiao")>1)
                {
                    if(confirm("在撤销之前，请您再确认一次，您真的要撤销选中的业务申请吗？")==true){}
                    else{return;}
                }
                if(objUrl.indexOf("action=logoff")>1)
                {
                    if(confirm("在注销之前，请您再确认一次，您真的要注销选中的业务记录吗？")==true){}
                    else{return;}
                }
                ...
                else if(objUrl.indexOf("xls")>1 || objUrl.indexOf("action=imp")>1 || objUrl.indexOf("action=tb_ldadd")>1)
                {
                    //alert(sourceUrl);
                    window.location.href = objUrl + "&sourceUrl="+sourceUrl+"&oNa="+objName;
                }

...
namespace join.pms.dal
{
    public class DataListBiz
    {
        ...
        private string GetPageNavStr(int pageRec)
        {
            ...
            pagestr = "";
            pagestr += PageNo > 1 ? "<a href=\"" + this.Url + "&searchRange=" + this.RangeKey + "&p=1&k=" + HttpUtility.UrlEncode(this.SearchKeys) + "\">首页</a><a href=\"" + this.Url + "&k=" + HttpUtility.UrlEncode(SearchKeys) + "&searchRange=" + this.RangeKey + "&p=" + pre + "\">上一页</a>" : "<span class=\"off\">首页</span><span class=\"off\">上一页</span>";
            ...
            pagestr += PageNo != allpage ? "<a href=\"" + this.Url + "&searchRange=" + this.RangeKey + "&p=" + next + "&k=" + HttpUtility.UrlEncode(SearchKeys) + "\">下一页</a><a href=\"" + this.Url + "&k=" + HttpUtility.UrlEncode(SearchKeys) + "&searchRange=" + this.RangeKey + "&p=" + allpage + "\">末页</a>" : "<span class=\"off\">下一页</span><span class=\"off\">末页</span>";
        ...
        public string GetList()
        {
            ...
            if (GetPageDataSet(ref errorMsg))
            {
                try
                {
                    #region this.FuncNo 2013/06/24 变更为只显示前10列的数据 02,06,11
                    if (this.FuncNo == "xxxx") { }
                    else
                    {
                    ...
                    //sb.Append("<tr><td class=\"small\" colspan=\"3\" height=\"23\">提示：通过多选操作可实现批量处理，点击功能按钮可进行相应的操作，例如点击“查找”可查找记录 ……</td></tr>");//if (a_FuncInfo[10] == "IsAdd") 
                    ...
                    //sb.Append(OprateButtonInfo(a_FuncInfo, a_FuncPowers));// 页头按钮及页码信息
                    //sb.Append("<div>");
                    ...
                    if (a_FuncInfo[13] == "IsEdit" || a_FuncInfo[14] == "IsDel") sb.Append("<th width=\"120\">操作</th>");
                    if (this.FuncNo == "xxxx")
                    {
                    ...
                    //if (a_FuncInfo[11] == "IsEdit" || a_FuncInfo[12] == "IsDel") sb.Append("<th width=\"130\">操作</th>");
                    sb.Append("</tr></thead>");
                    // ==================== 数据输出<tbody>  ================================================================
                    for (int i = 0; i < rowsNum; i++)
                    {
                        ...
                        //操作链接前置 2014/12/12 by Ysl
                        if (a_FuncPowers != null && a_FuncPowers.Length > 2)
                        {
                            // v_BizList,BizID,事项预约,IsCheck,4发起,#?action=add,6修改,#?action=edit,8删除,#?action=del,10查看,#?action=view,12 NoAdd,13 IsEdit,14 IsDel,15 IsView
                            ...
                            if (a_FuncInfo[13] == "IsEdit" || a_FuncInfo[14] == "IsDel" || a_FuncInfo[15] == "IsView")
                            {
                                if (a_FuncPowers[1] == "1" || a_FuncPowers[2] == "1")
                                {
                                    ...
                                    if (a_FuncPowers[1] == "1" && a_FuncInfo[13] == "IsEdit") sb.Append("<a class=\"edit\" title=\"修改……\" href=\"#\" onclick=\"javascript:SetBizUrl('" + a_FuncInfo[7] + "&k=" + objID + "','','edit');\"></a>");
                                    if (objAttribs == "1" || objAttribs == "6")
                                    {
                                        if (a_FuncPowers[2] == "1" && a_FuncInfo[14] == "IsDel" && IsMyBiz(objID, UserAreaCode)) sb.Append("<a class=\"but_audit\" title=\"审核…\" href=\"#\" onclick=\"javascript:SetBizUrl('" + a_FuncInfo[9] + "&k=" + objID + "','','edit');\"></a>");
                                    }
                                    if (a_FuncInfo[15] == "IsView")
                                    {
                                        sb.Append("<a class=\"but_view\" title=\"查看详细……\" href=\"#\" onclick=\"javascript:SetBizUrl('" + a_FuncInfo[11] + "&k=" + objID + "','','edit');\"></a>");
                                    }
                                    //sb.Append("<a class=\"but_data\" title=\"数据对比……\" href=\"#\" onclick=\"javascript:SetBizUrl('" + a_FuncInfo[11] + "&k=" + objID + "','','data');\"></a>");              
                                    sb.Append("</td>");
                                ...
                                {
                                    sb.Append("<td>");
                                    if (a_FuncInfo[15] == "IsView") sb.Append("<a class=\"but_view\" title=\"查看详细……\" href=\"#\" onclick=\"javascript:SetBizUrl('" + a_FuncInfo[11] + "&k=" + objID + "','','edit');\"></a>");
                                    sb.Append("&nbsp;</td>");
                                }
                            }
                            else { }
                        ...
                        for (int j = 0; j < colsNum; j++)
                        {
                            ...
                            if (a_Width[j] != "*")
                            {
                                if (a_Titles[j].IndexOf("身份证") > -1) { rowsLen = int.Parse(a_Width[j]); }
                                ...
                                else if (a_Titles[j].IndexOf("发起") > -1) { rowsLen = int.Parse(a_Width[j]); }
                                else if (a_Titles[j].IndexOf("申请") > -1) { rowsLen = int.Parse(a_Width[j]); }
                                ...
                                else if (a_Titles[j].IndexOf("状态") > -1) { rowsLen = int.Parse(a_Width[j]); }
                            ...
                            else
                            {
                                ...
                                {
                                    // 户籍地址 现居住地 m_AreaFilter
                                    if (a_Titles[j].IndexOf("地址") > -1 || a_Titles[j].IndexOf("居住地") > -1 || a_Titles[j].IndexOf("单位") > -1)
                                    {
        ...
        private string GetTabPageInfo(string[] funcInfo)
        {
            ...
            try
            {
                ...
                //sb.Append("<a href=\"#\" onclick=\"moveMe('advSearch');showSearchDiv('advSearch')\"><img src=\"images/btnL3Search.gif\" alt=\"查找 " + this.FuncName + "...\" title=\"查找 " + this.FuncName + "...\" border=\"0\" align=\"absmiddle\"></a>");
                sb.Append("</td>");
                sb.Append("</tr></tbody></table><!-- Tab Page End -->");
            }
            catch (Exception ex)
            {
                sb.Append("获取 Tab Pages 信息失败：" + ex.Message);
            }
        ...
        private string OprateButtonInfo(string[] funcInfo, string[] a_FuncPowers)
        {
            ...
            try
            {
                ...
                string[] aryNas;
                // 女方村/居,男方村/居,女方镇办,男方镇办,发证机关
                sb.Append("选择流程节点：<select name=\"viewRange\" id=\"viewRange\" class=\"small\" onchange=\"getQueryData(document.getElementById('viewRange').value,'" + this.Url + "')\">");

                if (RangeKey == "range_audit") { sb.Append("<option value=\"range_audit\" selected=\"selected\">正在处理中</option>"); }
                else { sb.Append("<option value=\"range_audit\">正在处理中</option>"); }

                if (RangeKey == "range_ok") { sb.Append("<option value=\"range_ok\" selected=\"selected\">审核通过(可发证)</option>"); }
                else { sb.Append("<option value=\"range_ok\">审核通过(可发证)</option>"); }

                if (RangeKey == "range_no") { sb.Append("<option value=\"range_no\" selected=\"selected\">审核驳回</option>"); }
                else { sb.Append("<option value=\"range_no\">审核驳回</option>"); }

                if (RangeKey == "range_all") { sb.Append("<option value=\"range_all\" selected=\"selected\">所有业务办理记录</option>"); }
                else { sb.Append("<option value=\"range_all\">所有业务办理记录</option>"); }

                if (RangeKey == "range_save") { sb.Append("<option value=\"range_save\" selected=\"selected\">所有已归档记录</option>"); }
                else { sb.Append("<option value=\"range_save\">所有已归档记录</option>"); }
                ...
                sb.Append("</select>&nbsp;功能操作：");
                // v_BizList,BizID,事项预约,IsCheck,发起,#?action=add,修改,#,审核,#,查看,#,NoAdd,IsEdit,IsDel,IsView
                ...
                // 催办按钮
                sb.Append("<input class=\"submit6 butOrange\" value=\"  催办  \" onclick=\"JavaScript:ChangeUrl('/siteadmin/CBMsgSend.aspx?action=add&BizID=" + BizID + "','SMS%e7%ae%a1%e7%90%86','');\" type=\"button\"/>&nbsp;");
            }
            catch (Exception ex)
            {
                sb.Append("获取操作按钮信息失败：" + ex.Message);

...
{
    public class DataListForUserWap
    {
                ...
                try
                {
                    ...
                    if (rowsNum > 0)
                    {
                        ...
                        for (int i = 0; i < rowsNum; i++)
                        {
                            ...
                            objID = dr[0]["" + this.PrimaryKey + ""].ToString();
                            objFieldsValue = FormatString(dr[i]["" + a_Fields[0] + ""].ToString(), a_Format[0]); // 格式化数据 
                            sb.Append("<div class=\"title\">" + FormatLink(objFieldsValue, objID, a_Link[0], objStatus) + "</div>");
                            sb.Append("<div class=\"time\">");
                            //数据开始
                            for (int j = 1; j < colsNum; j++)
                            {
                                objFieldsValue = FormatString(dr[i]["" + a_Fields[j] + ""].ToString(), a_Format[j]); // 格式化数据 
                                sb.Append(a_Titles[j] + ":" + objFieldsValue + "&nbsp;&nbsp;");
                            }
                            sb.Append("</div>");
